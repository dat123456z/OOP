    ...
    public static void EditExpense(DefaultTableModel model, Connection conn, JFrame frame, JTable table) {
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            while (newCategory.equals("Category")) {
                ...
                int retry = JOptionPane.showConfirmDialog(frame, message, "Edit Expense", JOptionPane.OK_CANCEL_OPTION);
            ...
            try {
                ...
                // Update database
                String updateSQL = "UPDATE expenses SET category = ?, description = ?, date = ?, amount = ? WHERE category = ? AND description = ? AND date = ? AND amount = ?";
                try (PreparedStatement pstmt = conn.prepareStatement(updateSQL)) {
                    pstmt.setString(1, newCategory);
                    pstmt.setString(2, newDescription);
                    pstmt.setDate(3, java.sql.Date.valueOf(newDate));
                    pstmt.setDouble(4, newAmount);
                    pstmt.setString(5, model.getValueAt(selectedRow, 0).toString());
                    pstmt.setString(6, model.getValueAt(selectedRow, 1).toString());
                    pstmt.setDate(7, java.sql.Date.valueOf(model.getValueAt(selectedRow, 2).toString()));
                    pstmt.setDouble(8, Double.parseDouble(model.getValueAt(selectedRow, 3).toString()));
                    pstmt.executeUpdate();
                }
            ...
            JOptionPane.showMessageDialog(frame, "Please select an expense to edit!", "Warning", JOptionPane.WARNING_MESSAGE);
        }
    }
}
